The `jwst_backgrounds` team performs a software release when necessary. We employ the following procedure for creating
a new release:
...
Make sure that your local version of the `master` branch is up-to-date. A new branch with the naming convention
vx.y.z should be opened off of the `master` branch, where vx.y.z is the version number of the release
(e.g. v0.4.1). This branch should be used for the changes described in the rest of this document.

2. Update the version number in `jwst_backgrounds/jwst_backgrounds/version.py/` using the x.y.z convention.
...
that points to `master` branch. Assign reviewers. Either you or the reviewer should eventually merge these pull
requests.
